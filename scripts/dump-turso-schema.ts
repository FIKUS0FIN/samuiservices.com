...
async function main() {
  try {
    const res = await client.execute(`SELECT type, name, sql FROM sqlite_master WHERE sql IS NOT NULL;`);
    let output = "";
    for (const row of res.rows) {
      output += row.sql + ";\n\n";
    }
    fs.writeFileSync('turso_schema.sql', output);
    console.log("Saved Turso schema to turso_schema.sql");
  } catch (err) {
    console.error(err);
  }
}
