      ...
      if (!existing) {
        ...
        await prisma.listing.create({
          data: {
            ...
            lat: b.lat,
            lng: b.lng,
            averageRating: (b as any).averageRating !== undefined ? (b as any).averageRating : Math.floor(Math.random() * (5 - 3 + 1) + 3) + Math.random(),
            reviewCount: (b as any).reviewCount !== undefined ? (b as any).reviewCount : Math.floor(Math.random() * 100) + 1,
